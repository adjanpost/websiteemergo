...
export default async function ProductDetailPage({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  ...
  return (
    <div className="pt-20">
      ...
      <section className="max-w-screen-xl mx-auto px-8 md:px-16 py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-[55fr_45fr] gap-10 md:gap-16 items-start">
          ...
          <div className="md:sticky md:top-24">
            <div className="mb-5">
              <p className="text-[10px] tracking-[0.3em] uppercase mb-4" style={{ fontFamily: "var(--font-sans)", color: "#c4956a" }}>
                {product.name}
              </p>
              <MadeInNL variant="detail" />
            </div>

            <h1 className="font-light leading-[1.05] mt-6 mb-3" style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(2rem, 3vw, 3rem)" }}>
              {product.tagline}
            </h1>
            <p className="italic text-muted-foreground mb-6" style={{ fontFamily: "var(--font-sans)", fontSize: "0.95rem" }}>
              {product.name}
            </p>

            <div style={{ width: 40, height: 1, backgroundColor: "#c4956a" }} className="mb-8" />

            <p className="font-light leading-[2] mb-10" style={{ fontFamily: "var(--font-sans)", fontSize: "0.88rem", maxWidth: "48ch" }}>
              {product.intro}
            </p>

            <div className="py-7 border-t border-b border-border mb-8">
              <OfferteButton href={`/${lang}/contact`} fullWidth label={d.common.requestQuote} />
              <Link
                href={`/${lang}/contact`}
              ...
              <p className="mt-4 text-[10px] text-muted-foreground" style={{ fontFamily: "var(--font-sans)" }}>
                {t.responseTime}
              </p>
            </div>
            ...
            <div className="divide-y divide-border">
              {product.specs.map((spec) => (
                <div key={spec.label} className="flex items-baseline justify-between py-4 gap-4">
                  <span className="text-[10px] tracking-[0.15em] uppercase text-muted-foreground shrink-0" style={{ fontFamily: "var(--font-sans)" }}>
                    {spec.label}
                  </span>
                  <span className="text-sm text-right" style={{ fontFamily: "var(--font-sans)" }}>
                    {spec.value}
                  </span>
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-28 items-start">
            <div className="space-y-6">
              {product.description.map((p, i) => (
                <p key={i} className="font-light leading-[1.9] text-muted-foreground" style={{ fontFamily: "var(--font-sans)", fontSize: "0.88rem", maxWidth: "52ch" }}>
                  {p}
                </p>
              ))}
            </div>
            <div>
              <blockquote className="font-light italic leading-snug whitespace-pre-line" style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(1.5rem, 2.5vw, 2rem)", color: "#2d4a2d" }}>
                {`"${product.quote}"`}
              </blockquote>
            </div>
          </div>
      ...
      <section className="py-36 md:py-48">
        <div className="max-w-screen-xl mx-auto px-8 md:px-16">
          <div className="mb-16">
            ...
            <h2 className="font-bold tracking-tight" style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)" }}>
              {t.materialsHeading}
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0 border-t border-border">
            {materialen.map((m) => (
              <div key={m.naam} className="pt-8 pb-8 pr-8">
                <h3 className="font-semibold mb-3" style={{ fontFamily: "var(--font-serif)", fontSize: "1.1rem" }}>
                  {m.naam}
                </h3>
                <p className="font-light text-muted-foreground leading-relaxed" style={{ fontFamily: "var(--font-sans)", fontSize: "0.8rem", maxWidth: "30ch" }}>
                  {m.tekst}
                </p>
              </div>
            ))}
      ...
      <section className="relative py-36 md:py-48" style={{ backgroundColor: "#1a2e1a" }} data-cursor="light">
        <div className="absolute inset-0" style={{ backgroundImage: `url(${related.overviewImage})`, backgroundSize: "cover", backgroundPosition: "center", opacity: 0.2 }} />
        <div className="relative z-10 max-w-screen-xl mx-auto px-8 md:px-16 text-center">
          <p className="text-[10px] tracking-[0.4em] uppercase text-white/40 mb-8" style={{ fontFamily: "var(--font-sans)" }}>
            {t.alsoView}
          </p>
          <h2 className="font-light italic text-white leading-[1.05] mb-6" style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(2rem, 4vw, 3.5rem)" }}>
            {related.name}
          </h2>
          <p className="font-light mb-12" style={{ fontFamily: "var(--font-sans)", fontSize: "0.9rem", color: "rgba(255,255,255,0.55)" }}>
            {related.tagline}
          </p>
          <Link
            href={`/${lang}/producten/${related.slug}`}
